...
class AssemblyConverter:
	...
	def I_type(
		...
		print("imm={} mod_imm={:d} {:x}".format(imm, mod_imm, mod_imm))
		return "".join([
	...
	def SB_type(
		...
		mod_imm12 = (int(imm) & 0x1000) >> 12
		mod_imm10 = (int(imm) & 0x7e0)  >> 5
		mod_imm4  = (int(imm) & 0x1e)   >> 1
		mod_imm11 = (int(imm) & 0x800)  >> 11

		mod_imm   = mod_imm12 << 6 | mod_imm10
		mod_imm_2 =  mod_imm4 << 1 | mod_imm11
...
#		mod_imm_2 += (int(imm) - ((int(imm) >> 11) << 11)) >> 10 # imm[4:1|11]
	...
	def UJ_type(
		...
		mod_imm20 = (int(imm) & 0x100000) >> 20 
		mod_imm10 = (int(imm) & 0x7fe   ) >> 1
		mod_imm11 = (int(imm) & 0x800   ) >> 11
		mod_imm19 = (int(imm) & 0xff000 ) >> 12
		mod_imm = mod_imm20 << 19 | mod_imm10 << 9 | mod_imm11 << 8 | mod_imm19
...
#		mod_imm += (int(imm) - ((int(imm) >> 19) << 19)) >> 12 # imm[20|10:1|11|19:12]
	...
	#READ FILE IN ADVANCE
	def __read_from_string(self, code_string):
		code = []
	...
	def __interpret(self,line,i):
		...
		if clean[0] == "sw" or clean[0] == "lw" or clean[0] == "lb" or clean[0] == "lbu" or clean[0] == "lh" or clean[0] == "sb" or clean[0] == "sh":
			#sw s0, 0(sp)
			w_spl = clean[2].split("(")
			clean[2] = w_spl[0]
		...
		if clean[0] in self.R_instr:
			...
			if clean[0] == "li": #need to consider larger than 12 bits
				...
				if int(clean[2]) > 2**11:
					...
					mod_imm = int(clean[2]) - ((int(clean[2])>>12)<<12) # imm[11:0]
					mod_imm_0 = mod_imm >> 1
					mod_imm_1 = mod_imm - mod_imm_0
					res.append(self.I_type("addi",self.__reg_map(clean[1]), mod_imm_0, self.__reg_map(clean[1])))
					res.append(self.I_type("addi",self.__reg_map(clean[1]), mod_imm_1, self.__reg_map(clean[1])))
				else:
	...
	def convert_from_string(self,code_string):
		self.code = self.__read_from_string(code_string)
		self.output_type = 'r'
		self.instructions = self.__get_instructions()

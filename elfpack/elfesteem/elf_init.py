...
from elfesteem.core.utils import force_bytes, force_str
...
class StrTable(Section):
    ...
    def parse_content(self, sex, size):
        ...
        while index < l:
            ...
            if p < 0:
                log.warning("Missing trailing 0 for string [%s]" % c + "index [%d]"%(index))  # XXX
                p = len(c) - index
...
class ProgramHeader(object):
    ...
    def __init__(self, parent, sex, size, phstr):
        ...
        if self.parent.parent.sh is None:
            return
...
class ProgramHeader64(object):
    ...
    def __init__(self, parent, sex, size, phstr):
        ...
        self.shlist = []
        if self.parent.parent.sh is not None:
            for s in self.parent.parent.sh:
                if isinstance(s, NullSection):
                    continue
                if ((isinstance(s, NoBitsSection) and s.sh.offset == self.ph.offset + self.ph.filesz) 
...
class virt(object):
    ...
    def get_rvaitem(self, start, stop=None):
        ...
        virt_item = []
        while total_len:
            #print("\tstart={:x} total_len={:x}".format(start, total_len))
            s = self.parent.getsectionbyvad(start)
            #print("\ts={}".format(s))
            if not s:
                s = self.parent.getphbyvad(start)
...
class NoSectionHeaderError(Exception):
    ...
    def __str__(self):
        return "No SectionHeader"

class NoProgramHeaderError(Exception):
    def __init__(self):
        pass

    def __str__(self):
        return "No ProgramHeader"

class ELF(object):

    def __init__(self, elfstr, size=None, sex=None, ignore_no_sechdr=False):
        self._content = elfstr
        self._size = size
        self._sex = sex
        self.ignore_no_sechdr = ignore_no_sechdr
    ...
    def is_in_virt_address(self, ad):
        if self.sh:
            ...
            for p in self.ph:
                if p.ph.vaddr <= ad < p.ph.vaddr + p.ph.memsz:
                    return True
